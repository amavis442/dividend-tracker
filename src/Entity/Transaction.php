...
#[
	ApiResource(
		normalizationContext: [
			'groups' => ['transaction:read', 'transaction:read:item'],
		],
...
class Transaction
{
	...
	public function validate(ExecutionContextInterface $context, $payload)
	{
		if (empty($this->getOriginalPrice()) && empty($this->getTotal())) {
			$context
				->buildViolation(
					'Original Price and/or total should be filled!'
				)
		...
		if (empty($this->total) || $this->total == 0) {
			$context
				->buildViolation('Total can not be empty or zero!')
				->atPath('total')
				->addViolation();
		}
	}
	...
	public function setSide(int $side): self
	{
		if (!in_array($side, [1, 2])) {
			throw new \OutOfBoundsException(
				'Value should be 1 for buy or 2 for sell.'
			);
		}
		$this->side = $side;
	...
	public function setShares(float $shares): self
	{
		$this->amount = $shares;
	...
	public function setTransactionDate(DateTimeInterface $transactionDate): self
	{
		if (!$transactionDate instanceof DateTime) {
			throw new \RuntimeException(
				'Transaction date is not of type DateTime:class'
			);
		}
	...
	/**
	 * Sets the transaction date, ensuring it is an instance of DateTime.
	 *
	 * @param DateTimeInterface $transactionDate The date of the transaction.
	 *
	 * @throws \RuntimeException If the provided date is not of type DateTime.
	 *
	 * @return self Returns the current Transaction instance for method chaining.
	 */
	public function setDate(DateTimeInterface $transactionDate): self
	{
		if (!$transactionDate instanceof DateTime) {
			throw new \RuntimeException(
				'Transaction date is not of type DateTime::class'
	...
	public function setOriginalPriceCurrency(
	...
	public function calcAllocation(): void
	{
		if ($this->total == 0.0) {
			throw new RuntimeException(
				'Only use this function when total has been set.'
			);
		}
	...
	public function calcPrice(): void
	{
		if ($this->originalPrice == 0.0 || $this->exchangeRate == 0.0) {
			throw new RuntimeException(
				'Only call this function when both original price and exchange rate are set'
			);
		}
	...
	public function setCurrencyOriginalPrice(
		?Currency $currencyOriginalPrice
	): static {
