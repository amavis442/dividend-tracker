	...
	private function getData(Collection $positions): array
	{
		...
		foreach ($positions as $position) {
			...
			if ($ticker->hasCalendar()) {
				...
				$row['tax'] = $ticker->getTax()
					? $ticker->getTax()->getTaxRate()
					: 0.15;
				$row['exchangerate'] = $this->dividendExchangeRateResolver->getRateForCalendar(
					$calendar
				);
				...
				$netDividend = $this->dividendService->getNetDividend(
					$position,
					$calendar
				);
				for ($m = 1; $m < 13; $m++) {
					...
					if ($dividendMonths->containsKey($m)) {
						$row[$indexBy] = round(
							$netDividend * (float) $position->getAmount(),
							2
						);
					}
				}
